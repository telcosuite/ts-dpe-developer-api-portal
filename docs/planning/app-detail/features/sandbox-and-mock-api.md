...
- AC1 (AC-sandbox-and-mock-api-01): Given a partner developer requests sandbox access for a product order API, when the request is submitted, then the portal validates subscription state, app owner, sandbox terms, scopes, and mock scenario availability before issuing test entitlement.
- AC2 (AC-sandbox-and-mock-api-02): Given a developer triggers a failed payment callback scenario, when the simulator runs, then the portal sends the configured event, records delivery status, exposes payload trace, and allows retry or DLQ inspection.
- AC3 (AC-sandbox-and-mock-api-03): Given an API governance lead updates a mock scenario after an OpenAPI change, when the scenario is republished, then the portal links updated test case, test data, environment, and compatibility notes to affected API versions.
- AC4 (AC-sandbox-and-mock-api-04): Given synthetic data contains regulated customer attributes, when the data pack is prepared, then the portal masks or removes sensitive fields, records privacy-safe generation evidence, and blocks export of prohibited values.
- AC5 (AC-sandbox-and-mock-api-05): Given sandbox rate limits are exceeded, when the developer continues testing, then the portal throttles mock calls, shows quota reset, and alerts platform operations for abnormal usage.
- AC6 (AC-sandbox-and-mock-api-06): Given a developer completes required mock scenarios, when the developer requests production certification, then the portal passes scenario run evidence and failures to the certification workflow.
